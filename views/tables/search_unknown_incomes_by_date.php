...
<div class="row justify-content-center">
    <div class="col-12 text-center">
        <?php if($target_date === false) { ?>
            <h1 class="h1 text-black">Buscar ingresos NO identificados por fecha</h1>
        <?php } else { ?>
            <h1 class="h1 text-black">Ingresos NO identificados del <?= $target_date->format('d/m/Y') ?></h1>
        <?php } ?>
    </div>    
    <div class="col-12 row justify-content-center px-4">
        <form class="col-12 row justify-content-center x_panel" method="POST" action="<?= $base_url ?>/views/tables/search_unknown_incomes_by_date.php">
    ...
    <?php if($target_date !== false) { ?>
        <?php
            ...
            $incomes = $unknown_model->GetUnknownIncomesByDate($target_date->format('Y-m-d'));
        ?>
        <div class="col-12 row justify-content-center px-4">
